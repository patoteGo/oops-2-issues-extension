...
describe("GitHubApi — uploader (A → B fallback)", () => {
	beforeEach(() => {
		vi.restoreAllMocks();
	...
	it("uses the user-attachment URL when Strategy A succeeds", async () => {
		...
		const u = api.uploader({
			token: "tok",
			owner: "me",
			repo: "repo",
			assetsRepo: "me/oops-assets",
			login: "me",
		});
		const file = await u.upload(blob, {
			filename: "shot.webp",
			description: "cap",
		});
	...
	it("falls back to Strategy B (Contents API) when the session is unavailable", async () => {
		...
		const file = await u.upload(blob, { filename: "shot.webp" });
	...
	it("upload with no filename defaults to oops.webp", async () => {
		...
		const file = await u.upload(blob, { description: "cap" });
		expect(file.url).toBe("https://github.com/user-attachments/assets/z");
		// policy carried the fixed oops.webp default filename
