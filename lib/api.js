 ...
 *  - uploader(ctx): the context-bound upload seam. The bound object's
 *    upload(...) FIRST tries GitHub's web "user-attachments" flow (Strategy
 *    A: POST /{owner}/{repo}/upload/policies/assets using the browser's
 *    github.com session cookie + a CSRF token), which renders inline on BOTH
 *    public and private repos and commits nothing. If A is unavailable (not
 *    logged in / GitHub changed the flow / any error), it falls back to
 *    Strategy B: upload to a designated PUBLIC assets repository via the
 *    Contents API, whose raw URL also renders inline regardless of the target
 *    repo's visibility.
 *
...
export class GitHubApi {
	...
	// ----- Upload: context-bound orchestrator --------------------------

	/**
	 * Bind an Account's upload context once; the returned object exposes the
	 * narrow upload seam. Screenshots omit `filename` (defaults to "oops.webp");
	 * references and recordings supply their own. One deep module, one place to
	 * test — the Strategy A→B fallback and the cross-instance caches live
	 * behind this interface, reached only through uploader(ctx).upload(...).
	 *
	 * @param {{token: string, owner: string, repo: string, assetsRepo?: string|null, login?: string|null}} ctx
	 * @returns {{upload(blob: Blob, opts?: {filename?: string, description?: string}): Promise<object>}}
	 */
	uploader(ctx) {
		const { token, owner, repo, assetsRepo = null, login = null } = ctx || {};
		return {
			upload: (blob, { filename = "oops.webp", description = "" } = {}) =>
				this._uploadWithFallback(
					token,
					owner,
					repo,
					blob,
	...
	/**
	 * @private Upload an image/file and return an inline-renderable URL.
	 * Strategy A (session user-attachments) first; on ANY failure falls back to
	 * Strategy B (Contents API into a public assets repo). The ambient context
	 * is bound by uploader(ctx); this is the implementation behind that seam.
	 */
	async _uploadWithFallback(
		token,
		owner,
		repo,
		blob,
