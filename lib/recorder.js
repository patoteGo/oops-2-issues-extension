/**
 * ScreenRecorder — oops 2 issues video capture engine (PRD: screen video recording).
 *
  ...
  return new Promise(resolve => {
    try {
      capture({ audio: true, video: false }, stream => {
        ...
        if (err) {
          console.debug('[oops 2 issues video debug]', 'recorder:tab-audio-error', {
            message: err.message,
          })
        }
        resolve(stream || null)
...
export class ScreenRecorder {
  ...
  async start() {
    ...
    try {
      ...
      console.debug('[oops 2 issues video debug]', 'recorder:mic-unavailable', {
        message: err?.message,
      })
    }
    ...
    console.debug('[oops 2 issues video debug]', 'recorder:start-streams', {
