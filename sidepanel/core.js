/**
 * BugSnap side panel — shared controller infrastructure.
 *
 * Owns the mutable `state`, the cached DOM (`el`), the EP API factory, and the
 * cross-cutting helpers every feature module needs (icons, status, busy
 * toggles, view switching, priority control, draft-storage primitives).
 *
...
export const DEFAULT_API = 'https://management.epsoftware.com.br'
export const SESSION_KEY = 'bugsnap:draft'
...
export const state = {
  ...
  uploaded: [], // already-uploaded files (e.g. saved videos) — attached as-is, no re-upload
  captureMode: 'full',
  priority: 'medium',
  checklist: [],
  previewOn: false,
...
export const api = () => new EpApi(state.apiBaseUrl)
...
export function showFormToast(kind, text, ms = 3500) {
  ...
  if (ms > 0) formToastTimer = setTimeout(() => (t.hidden = true), ms)
}
