 ...
 * through idle → recording → preview and asserts the view + pendingRecording
 * contract (the controller publishes the stopped-but-unsaved clip to state, as
 * Screenshots/References do). The state-machine rules are locked in
 * record-session.test.js — this covers the DOM/timer/recorder glue.
 */
import { describe, it, expect, vi, beforeEach } from "vitest";

// session.js / ui.js touch document at import time; stub their exports.
vi.mock("../session.js", () => ({ el: {}, state: { pendingRecording: null } }));
...
describe("record controller · start → stop → preview", () => {
	...
	it("moves to preview on stop and publishes the pending recording", async () => {
	...
	it("surfaces a start failure as an error status", async () => {
		...
		expect(state.pendingRecording).toBeNull();
	});
});
...
describe("record controller · cancel + re-record", () => {
	...
	it("re-record discards the first blob and restarts", async () => {
		...
		await ctrl.start();
		await ctrl.stop();
		expect(state.pendingRecording).not.toBeNull();
		await ctrl.reRecord();
